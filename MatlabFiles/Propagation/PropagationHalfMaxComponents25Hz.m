clear all

prompt = 'What is the time of onset?';
To = input(prompt)
% Open Data
[FileName,PathName] = uigetfile('*.nii','Select the Nifti file');
file=strcat(PathName,FileName)
B=MRIread(file);
D=double(B.vol);
clear B
S1=size(D);

[FileName,PathName] = uigetfile('*.mat','Select the region time series');
file2=strcat(PathName,FileName)
load(file2)
%load(strcat(file(1:size(file,2)-6),'TS.mat'))


D1=zeros(S1(1),S1(2),S1(3),1);
D2=zeros(S1(1),S1(2),S1(3),1);

for i=1:S1(4)
    Before=mean(TSo(To-50:To,i));
    StdBefore=std(TSo(To-50:To,i));
    MaxResponse=max(TSo(To:To+40,i));
    HalfMax=Before+(MaxResponse-Before)/2;
    if (MaxResponse-Before)>4*StdBefore
        [c index]=min(abs(TSo(To:(To+20),i)-HalfMax));
        Delay1(i)=index;
        D1=D1+D(:,:,:,i)*(20-Delay1(i))/20;
        D2=D2+D(:,:,:,i)*Delay1(i)/20;
    end      
end

D1m=Montage3(D1);
D2m=Montage3(D2);
D3=zeros(size(D2));
D3m=Montage3(D3);
Dm=cat(3,D1m,D2m,D3m);
Dm4norm=Dm;
Dm4norm(Dm==1)=0;
Sn=size(Dm4norm); 
M=prctile(reshape(Dm4norm,Sn(1)*Sn(2)*Sn(3),1),99.9);
fullFileName = fullfile(strcat(file(1:size(file,2)-4),'DelaysHalfMax.PNG'));
imwrite(Dm/M, fullFileName);



D1=zeros(S1(1),S1(2),S1(3),1);
D2=zeros(S1(1),S1(2),S1(3),1);

for i=1:S1(4)
    Before=mean(TSo(To-50:To,i));
    StdBefore=std(TSo(To-50:To,i));
    MaxResponse=max(TSo(To:To+40,i));
    HalfMax=Before+(MaxResponse-Before)/2;
    if (MaxResponse-Before)>3*StdBefore
        [c index]=min(abs(TSo(To:(To+35),i)-MaxResponse));
        Delay2(i)=index;
        D1=D1+D(:,:,:,i)*(35-Delay2(i))/35;
        D2=D2+D(:,:,:,i)*Delay2(i)/35;
    end      
end

D1m=Montage3(D1);
D2m=Montage3(D2);
D3=zeros(size(D2));
D3m=Montage3(D3);
Dm=cat(3,D1m,D2m,D3m);
Dm4norm=Dm;
Dm4norm(Dm==1)=0;
Sn=size(Dm4norm); 
M=prctile(reshape(Dm4norm,Sn(1)*Sn(2)*Sn(3),1),99.9);
fullFileName = fullfile(strcat(file(1:size(file,2)-4),'DelaysMax.PNG'));
imwrite(Dm/M, fullFileName);








