...
% The steps are: 
% * Detrending the data with the signal boxed averaged over 30 s. This allows to center the data and to remove decrease in fluorescence from fluorophore bleaching
...
% First detrend over N sec
Unbleached_data = Sdff*dFF(Data,N*Fr);

clear Data

out.vol=Unbleached_data(:,:,:,2:(S(4)-1));
err = MRIwrite(out,strcat(file(1:size(file,2)-4),'dFF',num2str(N),'s.nii'));
...
% 
out.vol=DM;
err = MRIwrite(out,strcat(file(1:size(file,2)-4),'dFF',num2str(N),'sM.nii'));
clear out
...
out.vol=Dpsf2;
err = MRIwrite(out,strcat(file(1:size(file,2)-4),'dFF',num2str(N),'sMpsf.nii'));
clear out
