...
D2(DleftrightN>0)=0;
out.vol=cat(4,D1,D2);
err = MRIwrite(out,strcat(file(1:size(file,2)-4),'LeftMinRightp.nii'));
...
fullFileName = fullfile(strcat(file(1:size(file,2)-4),'LeftMinRightMp.PNG'));
...
D1(DWalkN>0)=0;
out.vol=cat(4,D1,D2);
err = MRIwrite(out,strcat(file(1:size(file,2)-4),'RestWalkp.nii'));
...
Dm4norm(Dm==1)=0;
Sn=size(Dm4norm); M=prctile(reshape(Dm4norm,Sn(1)*Sn(2)*Sn(3),1),99.9);
fullFileName = fullfile(strcat(file(1:size(file,2)-4),'RestWalkMp.PNG'));
imwrite(Dm/M, fullFileName);
...
D1(DGroomN>0)=0;
out.vol=cat(4,D1,D2);
err = MRIwrite(out,strcat(file(1:size(file,2)-4),'RestGroomp.nii'));
...
Dm4norm(Dm==1)=0;
Sn=size(Dm4norm); M=prctile(reshape(Dm4norm,Sn(1)*Sn(2)*Sn(3),1),99.9);
fullFileName = fullfile(strcat(file(1:size(file,2)-4),'RestGroomMp.PNG'));
imwrite(Dm/M, fullFileName);
...
Dm4norm=DmGW;
Dm4norm(DmGW==1)=0;
M=max(max(max(Dm4norm)));
fullFileName = fullfile(strcat(file(1:size(file,2)-4),'WalkGroomMp.PNG'));
