...
[B SD L D] = spca(R, [], Npc, inf,-5000);

%% Use the following lines if want to use melodic after a more efficient SVD than melodic itself
...
% save PCA maps
out4.vol = Dpca;
err = MRIwrite(out4,strcat(file(1:size(file,2)-4),'50PC_-5000.nii'));


%% ICA
[icasig, A, W] = fastica (B','approach','symm','epsilon', 0.001);
...
save(strcat(file(1:size(file,2)-4),'TS'),'TSo')
