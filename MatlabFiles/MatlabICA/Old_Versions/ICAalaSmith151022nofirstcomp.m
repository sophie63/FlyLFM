...
% save PCA maps
out4.vol = Dpca;
err = MRIwrite(out4,'/home/sophie/Desktop/Av863_sv30.nii');
...
GM=GM.*repmat(sign(GMzpm+GMznm),size(GM,1),1)./repmat(std(GM),size(GM,1),1); 
...
TS=u*s*A;
TS=TS.*repmat(sign(GMzpm+GMznm),size(TS,1),1);
...
out.vol = Dica;
%out.vol=D*100000/P;
err = MRIwrite(out,'/home/sophie/Desktop/863matlab_ICs75nofirstcomp.nii');
...
l=2
    parfor k=1:S1(4)
        for j=1:S2(1)
            Correction(k,l)=Correction(k,l)+R(k,j)*Dicaantiz(j,l);           
        end
    end
   l
%end
TScorrect=TS-Correction;
...
%Only first components
NewR2=TSo(:,65:75)*GMo(:,65:75)';
parfor i=1:S1(4)
NewD2(:,:,:,i)=reshape(NewR2(i,:),[S1(1),S1(2),S1(3)]);
end
out4.vol = NewD2;
%out.vol=D*100000/P;
err = MRIwrite(out4,'/home/sophie/Desktop/863recfirstcomp.nii');

%Data without first components
NewR3=R-NewR2;
parfor i=1:S1(4)
NewD3(:,:,:,i)=reshape(NewR3(i,:),[S1(1),S1(2),S1(3)]);
end
out5.vol = NewD3;
%out.vol=D*100000/P;
err = MRIwrite(out5,'/home/sophie/Desktop/863recnofirstcomp.nii');